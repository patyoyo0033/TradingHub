<x-app-layout>
    ...
    @section('page_description', 'Update your trade execution details and psychology.')
    ...
    <div class="bg-surface-container-highest/60 backdrop-blur-[30px] rounded-xl p-6 md:p-8 shadow-[0_12px_32px_rgba(11,19,38,0.4)] border-t border-on-surface/10 relative overflow-hidden max-w-4xl mx-auto">
        ...
        <form action="{{ route('trades.update', $trade) }}" method="POST" enctype="multipart/form-data" class="relative z-10 space-y-8">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex gap-4 h-[46px]">
                            <label class="flex-1 relative cursor-pointer group">
                                <input type="radio" name="direction" value="BUY" class="peer sr-only" {{ old('direction', $trade->direction) == 'BUY' ? 'checked' : '' }} required>
                            ...
                            <label class="flex-1 relative cursor-pointer group">
                                <input type="radio" name="direction" value="SELL" class="peer sr-only" {{ old('direction', $trade->direction) == 'SELL' ? 'checked' : '' }} required>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="session" required class="w-full bg-surface-container-lowest text-black border border-outline-variant/30 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-container transition-all text-sm appearance-none">
                            ...
                            <option value="Asian" {{ old('session', $trade->session) == 'Asian' ? 'selected' : '' }}>Asian</option>
                            <option value="London" {{ old('session', $trade->session) == 'London' ? 'selected' : '' }}>London</option>
                            <option value="New York" {{ old('session', $trade->session) == 'New York' ? 'selected' : '' }}>New York</option>
                            <option value="Overlap" {{ old('session', $trade->session) == 'Overlap' ? 'selected' : '' }}>Overlap</option>
                        </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <input name="duration_minutes" value="{{ old('duration_minutes', $trade->duration_minutes) }}" class="w-full bg-surface-container-lowest text-black border border-outline-variant/30 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-container transition-all font-mono text-sm" placeholder="e.g. 120" type="number"/>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div class="space-y-2">
                        ...
                        <input name="lot_size" value="{{ old('lot_size', $trade->lot_size) }}" class="w-full bg-surface-container-lowest text-black border border-outline-variant/30 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-container transition-all font-mono text-sm" placeholder="1.00" step="any" type="number"/>
                    ...
                    <div class="space-y-2">
                        ...
                        <input name="entry_price" value="{{ old('entry_price', $trade->entry_price) }}" required class="w-full bg-surface-container-lowest text-black border border-outline-variant/30 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-container transition-all font-mono text-sm" placeholder="0.0000" step="any" type="number"/>
                    ...
                    <div class="space-y-2">
                        ...
                        <input name="tp_price" value="{{ old('tp_price', $trade->tp_price) }}" class="w-full bg-surface-container-lowest text-black border border-outline-variant/30 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-secondary transition-all font-mono text-sm" placeholder="0.0000" step="any" type="number"/>
                    </div>
                    
                    <div class="space-y-2">
                        <label class="block text-xs font-medium text-tertiary uppercase tracking-wider">Stop Loss</label>
                        <input name="sl_price" value="{{ old('sl_price', $trade->sl_price) }}" class="w-full bg-surface-container-lowest text-black border border-outline-variant/30 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-tertiary transition-all font-mono text-sm" placeholder="0.0000" step="any" type="number"/>
                    </div>
                    
                    <div class="space-y-2 md:col-span-2">
                        <label class="block text-xs font-medium text-black-variant uppercase tracking-wider">Realized PNL ($)</label>
                        <input name="pnl_amount" value="{{ old('pnl_amount', $trade->pnl_amount) }}" required class="w-full bg-surface-container-lowest text-black border border-outline-variant/30 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-container transition-all font-mono text-lg" placeholder="-150.00" step="any" type="number"/>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <select name="strategy_tags[]" multiple class="w-full bg-surface-container-lowest text-black border border-outline-variant/30 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-container transition-all text-sm h-24">
                            @php $oldTags = old('strategy_tags', is_array($trade->strategy_tags) ? $trade->strategy_tags : []); @endphp
                    ...
                    <div class="space-y-6">
                        <div class="space-y-2">
                            ...
                            <select name="setup_quality" class="w-full bg-surface-container-lowest text-black border border-outline-variant/30 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-container transition-all text-sm appearance-none">
                                ...
                                <option value="5" {{ old('setup_quality', $trade->setup_quality) == '5' ? 'selected' : '' }}>A+ (Perfect Setup)</option>
                                <option value="4" {{ old('setup_quality', $trade->setup_quality) == '4' ? 'selected' : '' }}>A (High Probability)</option>
                                <option value="3" {{ old('setup_quality', $trade->setup_quality) == '3' ? 'selected' : '' }}>B (Average)</option>
                                <option value="2" {{ old('setup_quality', $trade->setup_quality) == '2' ? 'selected' : '' }}>C (Low Probability)</option>
                                <option value="1" {{ old('setup_quality', $trade->setup_quality) == '1' ? 'selected' : '' }}>D (Gambling)</option>
                            </select>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="emotion" class="w-full bg-surface-container-lowest text-black border border-outline-variant/30 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-container transition-all text-sm appearance-none">
                                ...
                                <option value="Neutral" {{ old('emotion', $trade->emotion) == 'Neutral' ? 'selected' : '' }}>Neutral / Focused</option>
                                <option value="Confident" {{ old('emotion', $trade->emotion) == 'Confident' ? 'selected' : '' }}>Confident</option>
                                <option value="FOMO" {{ old('emotion', $trade->emotion) == 'FOMO' ? 'selected' : '' }}>FOMO</option>
                                <option value="Revenge" {{ old('emotion', $trade->emotion) == 'Revenge' ? 'selected' : '' }}>Revenge Trading</option>
                                <option value="Anxious" {{ old('emotion', $trade->emotion) == 'Anxious' ? 'selected' : '' }}>Anxious / Fearful</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2 md:col-span-2">
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <input type="checkbox" name="mistake_flag" value="1" {{ old('mistake_flag', $trade->mistake_flag) ? 'checked' : '' }} class="w-5 h-5 rounded border-outline-variant/30 bg-surface-container-lowest text-tertiary focus:ring-tertiary/50">
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <textarea name="emotion_notes" class="w-full bg-surface-container-lowest text-black border border-outline-variant/30 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-container transition-all text-sm resize-none" placeholder="What were you feeling during execution? Did you follow your rules?" rows="4">{{ old('emotion_notes', $trade->emotion_notes) }}</textarea>
                        @error('emotion_notes') <span class="text-error text-xs">{{ $message }}</span> @enderror
                    </div>

                    <div class="space-y-2 md:col-span-2">
                        <label class="block text-xs font-medium text-black-variant uppercase tracking-wider">Chart Screenshot (Replace)</label>
                        
                        @if($trade->image_path)
                            <div class="mb-3 relative w-48 rounded-lg overflow-hidden border border-outline-variant/30">
                                <img src="{{ Storage::url($trade->image_path) }}" alt="Current Chart" class="w-full object-cover">
                            </div>
                        @endif
